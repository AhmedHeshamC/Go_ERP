...
type AdjustInventoryRequest struct {
	...
	WarehouseID  uuid.UUID `json:"warehouse_id" binding:"required,uuid"`
...
type ReleaseInventoryRequest struct {
	ReservationID uuid.UUID `json:"reservation_id" binding:"required,uuid"`
	Quantity      int       `json:"quantity" binding:"required,min=1"`
	Reason        string    `json:"reason" binding:"required,min=1,max=500"`
}
...
type TransferInventoryRequest struct {
	ProductID     uuid.UUID `json:"product_id" binding:"required,uuid"`
	FromWarehouseID uuid.UUID `json:"from_warehouse_id" binding:"required,uuid"`
	ToWarehouseID   uuid.UUID `json:"to_warehouse_id" binding:"required,uuid"`
	Quantity        int       `json:"quantity" binding:"required,min=1"`
	Reason          string    `json:"reason" binding:"required,min=1,max=500"`
	ReferenceID     *uuid.UUID `json:"reference_id,omitempty"`
...
type ApproveTransactionRequest struct {
	ApprovedBy uuid.UUID `json:"approved_by" binding:"required,uuid"`
...
type LowStockAlertRequest struct {
	ProductID   *uuid.UUID `json:"product_id,omitempty" binding:"omitempty,uuid"`
	WarehouseID *uuid.UUID `json:"warehouse_id,omitempty" binding:"omitempty,uuid"`
