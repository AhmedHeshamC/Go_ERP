...
func testAPIPerformanceUnderLoad(t *testing.T) {
	...
	const (
		...
		maxErrorRate      = 0.01 // 1% (relaxed for testing)
...
func testMemoryLeakDetection(t *testing.T) {
	// Use shorter duration for automated testing
	// For production validation, set testDuration to 60 * time.Minute
	testDuration := 2 * time.Minute
	t.Log("Running memory leak detection test (2 minutes for testing, use 1 hour in production)...")
