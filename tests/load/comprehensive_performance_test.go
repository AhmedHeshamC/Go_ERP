...
// TestComprehensivePerformance runs all performance tests
...
func testMemoryLeakDetection(t *testing.T) {
	// For testing purposes, we'll use a shorter duration
	// In production, this should run for the full hour
	shortTest := testing.Short()

	var testDuration time.Duration
	if shortTest {
		testDuration = 2 * time.Minute // Short test: 2 minutes
		t.Log("Running short memory leak detection test (2 minutes)...")
	} else {
		testDuration = 60 * time.Minute // Full test: 1 hour
		t.Log("Running full memory leak detection test (1 hour)...")
	}
